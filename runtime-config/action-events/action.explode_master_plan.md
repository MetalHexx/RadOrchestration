...
completion_when: The script parses the Master Plan cleanly with no data.error.
alternate_outcomes:
  - event: explosion_failed
    when: The script returns ok true with data.error present, the parse-failure envelope shape.
---
...
If the project `phases/` or `tasks/` already contain anything, the script clears their contents before re-seeding.

Disambiguate the result by envelope shape: `ok: true` with `data.error` present is a parse failure. `ok: false` with `error.type: system_error` is a real failure — route through `rad-log-error` instead.
