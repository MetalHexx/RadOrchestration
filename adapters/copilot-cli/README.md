...
| Agent frontmatter projection (lowercase tools, dot-versioned model id, `target: github-copilot`, single-string model) | §3.A |
...
- **`target: github-copilot` marks the agent for the CLI environment**
  (research §3.A). VS Code-only agents would set `target: vscode`; the
  Copilot CLI adapter explicitly opts into the CLI/cloud-agent path.
