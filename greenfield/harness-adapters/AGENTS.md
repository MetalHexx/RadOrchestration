...
- **`filenames`** (function or object) — Transforms source filenames. Maps a source filename (e.g., `skill-name.md`) to its harness-specific output filename (e.g., `skill-name.claude.md` or `skill-name.copilot-vscode.md`). Per-harness naming rules (suffix, prefix, or full rewrite) are adapter-owned.
- **`bodyTokens`** (function or object) — Transforms body text. Maps placeholder tokens (e.g., `${TOOL_NAME}` for tool namespace differences) to harness-specific values. Common tokens include tool-name prefixes, model identifiers, and framework references. Token definition and substitution are adapter-owned.
...
3. In the canonical `harness-files/agents/` directory, author one `<agent-name>.<new-harness>.yml` body file for each agent that requires harness-specific body content (e.g., different tool names or model aliases). These are optional; if no harness-specific body exists for an agent, the engine uses the canonical `.md` body.
