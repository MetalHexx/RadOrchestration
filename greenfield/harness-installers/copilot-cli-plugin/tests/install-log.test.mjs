...
test('appendInstallLog writes JSONL with the five canonical fields and channel copilot-cli-plugin', () => {
  const dir = fs.mkdtempSync(join(os.tmpdir(), 'log-cli-'));
  try {
    ...
    assert.strictEqual(obj.channel, 'copilot-cli-plugin');
    assert.strictEqual(obj.action, 'fresh-install');
  } finally { fs.rmSync(dir, { recursive: true, force: true }); }
});

test('appendInstallLog is best-effort — write to an unreachable path resolves without throwing', async () => {
