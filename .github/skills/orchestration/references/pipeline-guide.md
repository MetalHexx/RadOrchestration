...
| 6 | `create_phase_plan` | Agent spawn | Spawn **Tactical Planner** (phase plan mode) for `result.context.phase`. If `result.context.is_correction` is true, create a corrective phase plan using `result.context.previous_review`. Output: PHASE-PLAN.md | `phase_plan_created` with `{ "doc_path": "<output-path>" }` |
