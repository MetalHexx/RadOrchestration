...
| 22 | `invoke_memory_ingest` | Agent spawn | First, always signal `memory_ingest_requested` to record the intent. Then check `memory.enabled` and `auto_ingest` config. If `auto_ingest` = `never` or `memory.enabled` = `false`: signal `memory_ingest_completed --success false` (silent skip). If `auto_ingest` = `ask`: present human gate "Ingest project into knowledge base?" — if declined, signal `memory_ingest_completed --success false`; if approved, proceed. Spawn a generic subagent loaded with the `manage-memory` skill in pipeline mode. The subagent ingests the project directory via `kb_ingest_dir`. | `memory_ingest_completed --success <true\|false> [--error <message>]` |
...
| `memory_ingest_requested` | *(none)* | Validation checkpoint when pipeline reaches COMPLETE tier and `pipeline.memory_ingested` is false. |
| `memory_ingest_completed` | `--success <true\|false> [--error <message>]` | After memory ingest subagent completes (success or failure) or after skip decision. Always signal — never omit. |
