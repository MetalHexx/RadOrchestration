...
(async () => {
  ...
  await test('printHelp(".agents") — usage line uses ".agents" prefix and no .github reference', async () => {
    ...
    assert.ok(
      out.includes('node .agents/skills/validate-orchestration/scripts/validate-orchestration.js'),
  ...
  await test('printHelp(".agents") — 6 occurrences of ".agents/skills/validate-orchestration" (1 usage + 5 examples)', async () => {
    const out = captureStdout(() => printHelp('.agents'));
    const count = (out.match(/\.agents\/skills\/validate-orchestration/g) || []).length;
    assert.strictEqual(count, 6,
      `Expected 6 occurrences of ".agents/skills/validate-orchestration" but found ${count}`
    );
  });
  ...
  await test('printHelp() — no argument falls back to ".github" (backward compatibility)', async () => {
    ...
    assert.ok(
      out.includes('node .github/skills/validate-orchestration/scripts/validate-orchestration.js'),
  ...
  await test('printHelp(".copilot") — arbitrary root is substituted correctly', async () => {
    ...
    assert.ok(
      out.includes('.copilot/skills/validate-orchestration/scripts/validate-orchestration.js'),
