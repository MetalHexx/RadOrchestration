...
(async () => {
  ...
  await test('orchRoot parameter changes config file path', async () => {
    ...
    assert.ok(
      calledPath.endsWith(path.join('.agents', 'orchestration.yml')),
      `Expected path ending with ${path.join('.agents', 'orchestration.yml')}, got "${calledPath}"`
    );
    assert.ok(
  ...
  await test('orchRoot defaults to ".github" when undefined', async () => {
    ...
    assert.ok(calledPath !== null, 'readFile should have been called');
    assert.ok(
      calledPath.endsWith(path.join('.github', 'orchestration.yml')),
      `Expected path ending with ${path.join('.github', 'orchestration.yml')}, got "${calledPath}"`
    );
  });
