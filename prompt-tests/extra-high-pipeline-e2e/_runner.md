# Extra-High Pipeline E2E — Runner Prompt
...
| Input | Default | Notes |
|-------|---------|-------|
| Fixture name | `rainbow-hello` | Matches `prompt-tests/extra-high-pipeline-e2e/fixtures/<fixture>/BRAINSTORMING.md` |
...
2. Create the run folder (named after the project name) and the three subdirs the explosion script will write into:
   ```
   prompt-tests/extra-high-pipeline-e2e/output/<fixture>/<PROJECT-NAME>/
     ├── phases/
     ├── tasks/
     └── backups/
   ```
...
3. Copy the fixture brainstorming doc into the run folder, renaming it to match project naming conventions (`<PROJECT-NAME>-BRAINSTORMING.md`):
   ```
   prompt-tests/extra-high-pipeline-e2e/fixtures/<fixture>/BRAINSTORMING.md
     → prompt-tests/extra-high-pipeline-e2e/output/<fixture>/<PROJECT-NAME>/<PROJECT-NAME>-BRAINSTORMING.md
   ```
