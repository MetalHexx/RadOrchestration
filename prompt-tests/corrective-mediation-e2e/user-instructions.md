...
Open `http://localhost:3000`. The UI reads projects from `~/.radorc/projects`. For this harness run, the output folder lives under `prompt-tests/corrective-mediation-e2e/output/` — symlink or move the run-specific project folder into the resolved projects directory so the UI can see it. Then verify:
