...
3. Set your `<run-folder>` variable — every `radorch pipeline signal` call uses `--project-dir <run-folder>` AND `--config <run-folder>/orchestration.yml`. The fixture ships a local `orchestration.yml` with `auto_commit: never` and `auto_pr: never` so the conditional `commit_gate` and `pr_gate` route to their `false` branches. Without `--config`, the engine falls back to the global `~/.radorc/orchestration.yml` which has `auto_commit: ask` and the corrective cycle will request a real commit — not what the harness wants.
