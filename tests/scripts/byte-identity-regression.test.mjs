...
//
// scripts/package-lock.json is also skipped: it is regenerated by `npm install`
// before sync-source.js runs (both locally and in CI), and its bytes vary by
// platform / Node / npm version (e.g. OS-specific optional dependency entries).
// The lockfile is generated, not authored, so byte-identity drift on it is
// noise rather than meaningful regression.
function isBuildArtifactPath(k) {
  return /^skills\/rad-orchestration\/scripts\/(dist|dist-bundle|node_modules)\//.test(k) ||
         ...
         k === 'skills/rad-orchestration/scripts/package-lock.json';
}
