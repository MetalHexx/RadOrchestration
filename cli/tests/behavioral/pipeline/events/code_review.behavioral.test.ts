...
// State after task_completed: task_executor=completed, code_review=not_started.
const afterTaskCompletedState = {
...
describe('code_review_started event (FR-3, FR-7, DD-2)', () => {
  it('code_review_started marks code_review in_progress and returns action=spawn_code_reviewer', async () => {
    const w = buildWorld({
      template: { id: 'syn-exec', body: EXECUTION_TEMPLATE_BODY },
      state: afterTaskCompletedState,
    ...
    const env = await captureEnvelope(async () => {
      await runCommand(pipelineSignalCommand, {
        argv: ['--event', 'code_review_started', '--phase', '1', '--task', '1', '--project-dir', w.projectDir, '--config', w.configPath],
