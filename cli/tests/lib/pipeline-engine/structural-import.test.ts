...
import schemaJson from '../../../src/lib/pipeline-engine/schemas/orchestration-state-v6.schema.json' with { type: 'json' };
...
describe('engine library lands at cli/src/lib/pipeline-engine/', () => {
  ...
  it('schema artifact resolves through the sibling schemas/ folder', () => {
    expect((schemaJson as { $id?: string }).$id ?? '').toContain('orchestration-state-v6');
  });
