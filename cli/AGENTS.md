...
- **`cli/src/lib/pipeline/`** — the pipeline engine lives under the CLI's `lib/` tree as a shared library. `commands/pipeline/signal.ts` is the entry layer that projects engine results into the canonical envelope; `commands/gate/shared.ts` is the second consumer, driving `processEvent` for plan and final approval. New subcommands should not reach into the engine surface unless they are approving or progressing pipeline state.
