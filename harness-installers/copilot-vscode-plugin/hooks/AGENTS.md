...
Runs on the user's first prompt after plugin installation. Calls `runInstall({ pluginRoot, radHome })` from `lib/install/run-install.js`. **On success**, atomically rewrites the plugin's own `hooks/hooks.json` (tmp + rename) to delete its `UserPromptSubmit` entry — so the hook never fires again until the next plugin upgrade ships a fresh `hooks.json` with `UserPromptSubmit` restored. `SessionStart` (drift-check) stays in place. **On failure**, `hooks.json` is left untouched and the bootstrap exits 1 (non-blocking for VS Code per NFR-12) — VS Code's normal hook dispatch fires bootstrap again on the next prompt, naturally retrying.
