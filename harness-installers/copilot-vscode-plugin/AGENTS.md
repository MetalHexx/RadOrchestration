...
| Hook dispatch layer | inline `node -e` shim in `hooks.json` reads `${CLAUDE_PLUGIN_ROOT}` (Claude Code injects it) | inline `node -e` shim in `hooks.json` probes a chain of plugin-root env vars (`COPILOT_PLUGIN_ROOT` → `COPILOT_VSCODE_PLUGIN_ROOT` → `CLAUDE_PLUGIN_ROOT` → `PLUGIN_ROOT`); docs are silent on which (if any) VS Code injects for Copilot-format plugins (§5 #1 of `docs/research/copilot-vscode-plugin-system.md`), so the shim emits a stderr diagnostic listing matching `process.env` keys if none resolve |
