...
// invokes the copilot-vscode-plugin installer's `runBuild()` against it.
// Returns the resolved per-installer `output/` directory so tests can assert
// on what the build emitted.
//
...
function stageFixture(root) {
  ...
  // installer source: hooks/, .claude-plugin/plugin.json (vscode uses claude-format manifest layout)
  const installerSrc = path.join(root, 'harness-installers/copilot-vscode-plugin');
  ...
  fs.writeFileSync(path.join(installerSrc, '.claude-plugin/plugin.json'),
    JSON.stringify({
      name: 'rad-orc-vscode', version: '1.0.0-alpha.14',
...
function seedRealManifest(root) {
  const fixtureManifests = path.join(root, 'harness-installers/copilot-vscode-plugin/manifests');
  ...
  const realRoot = path.join(REPO_ROOT, 'harness-installers/copilot-vscode-plugin');
...
export async function runBuild(opts = {}) {
  ...
  const outRoot = path.join(fixtureRoot, 'harness-installers/copilot-vscode-plugin/output');
  const cleanup = () => {
