...
test('every committed manifest carries version, channel, and a files array (FR-34, DD-12)', () => {
  ...
  for (const f of files) {
    ...
    assert.strictEqual(m.channel, 'copilot-vscode-plugin', `${f}: channel matches DD-12`);
...
test('orchestration.yml entry carries user-config ownership (FR-13)', () => {
  const files = fs.readdirSync(MANIFESTS_DIR).filter((f) => /^v.+\.json$/.test(f));
  for (const f of files) {
    const m = JSON.parse(fs.readFileSync(path.join(MANIFESTS_DIR, f), 'utf8'));
