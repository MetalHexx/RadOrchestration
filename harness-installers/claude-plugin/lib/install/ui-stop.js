...
//
// Returns a structured outcome so callers can decide whether to proceed with
// the install (wasRunning=false OR stopped=true) or abort with a clear
// message naming the live UI status (wasRunning=true AND stopped=false).
...
/**
 * Build the human-readable abort message for stop-failure cases. Used by
 * run-install to throw a UiLockError with a message the bootstrap can surface
...
export function formatUiLockMessage(status, reason) {
  return (
    ...
    `${reason}. Stop it manually and re-submit a prompt to retry the install.`
  );
}
