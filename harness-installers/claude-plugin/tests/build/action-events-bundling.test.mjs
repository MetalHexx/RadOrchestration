// harness-installers/claude-plugin/tests/build/action-events-bundling.test.mjs —
// FR-1, FR-19, FR-20, AD-3, AD-10. Asserts the claude-plugin installer build
// stages `runtime-config/action-events/` under `output/_install-source/`
...
test('claude-plugin build stages action-events under _install-source/ (FR-19)', async () => {
...
test('claude-plugin build ships an empty action-events/custom/ — no user-authored files leak (FR-20)', async () => {
  const { outRoot, cleanup } = await runBuild();
  try {
