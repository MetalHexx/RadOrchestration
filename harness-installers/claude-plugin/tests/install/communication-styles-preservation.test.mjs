// harness-installers/claude-plugin/tests/install/communication-styles-preservation.test.mjs —
// Asserts the claude-plugin's manifest-driven install/upgrade preserves user
// payloads under communication-styles/custom/:
//   - shipped style files land under ${RAD_HOME}/communication-styles/
//   - any user-authored file under custom/ survives the remove half of upgrade
//   - removeManifestFiles refuses any entry under communication-styles/custom/
//     (the slot is user-owned and ships empty)
//   - a user's `communication_style` selection in orchestration.yml survives
//     remove (the config file is seeded once, never overwritten)
