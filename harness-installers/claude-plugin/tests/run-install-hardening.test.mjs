...
async function makePluginRoot(version) {
  const dir = fs.mkdtempSync(join(os.tmpdir(), 'plugin-claude-hardening-'));
  ...
  fs.writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: '@rad-orchestration/claude-plugin', version }));
  ...
  fs.writeFileSync(join(dir, `manifests/v${version}.json`), JSON.stringify({
    version, channel: 'claude-plugin', files: [
...
test('install.json upsert on NOOP when entry shape is drifted', async () => {
  ...
  try {
    ...
    fs.writeFileSync(join(radHome, 'install.json'), JSON.stringify({
      harnesses: {
        'claude-plugin': { version: '1.0.0', channel: 'claude-plugin', installed_at: '2020-01-01T00:00:00.000Z' /* last_writer_version missing */ },
      },
    }, null, 2));
    const result = await runInstall({
      pluginRoot, radHome,
    ...
    assert.strictEqual(ij.harnesses['claude-plugin'].last_writer_version, '1.0.0');
  } finally {
    fs.rmSync(radHome, { recursive: true, force: true });
    fs.rmSync(pluginRoot, { recursive: true, force: true });
  }
...
test('install.json upsert on downgrade-noop when entry shape is drifted', async () => {
  ...
  try {
    ...
    fs.mkdirSync(radHome, { recursive: true });
    fs.writeFileSync(join(radHome, 'install.json'), JSON.stringify({
      harnesses: { 'claude-plugin': { version: '1.1.0', channel: 'wrong-channel', installed_at: '2020-01-01T00:00:00.000Z', last_writer_version: '1.1.0' } },
    }, null, 2));
    const result = await runInstall({
    ...
    assert.strictEqual(result.installJsonUpserted, true);
    const ij = JSON.parse(fs.readFileSync(join(radHome, 'install.json'), 'utf8'));
    assert.strictEqual(ij.harnesses['claude-plugin'].channel, 'claude-plugin');
    // Downgrade-noop preserves the installed (higher) version, not the delivering one.
    assert.strictEqual(ij.harnesses['claude-plugin'].version, '1.1.0');
  } finally {
    fs.rmSync(radHome, { recursive: true, force: true });
    fs.rmSync(pluginRoot, { recursive: true, force: true });
  }
