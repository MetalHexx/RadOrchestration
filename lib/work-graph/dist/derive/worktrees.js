...
const defaultExec = (file, args, opts) => execFileSync(file, args, { cwd: opts.cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
function listWorktrees(exec, cwd) {
    const out = new Map();
    let text = '';
    try {
        text = exec('git', ['worktree', 'list', '--porcelain'], { cwd });
    }
    catch {
        return out;
    }
...
export function resolveWorktrees(projectName, deps) {
    ...
    let sc;
    try {
        sc = JSON.parse(fs.readFileSync(statePath, 'utf8'))?.pipeline?.source_control;
    }
    catch {
        return [];
    }
    ...
    const live = listWorktrees(exec, wtPath);
    const key = path.resolve(wtPath);
    return [{ repo: path.basename(wtPath), path: wtPath, branch: typeof sc.branch === 'string' ? sc.branch : null, exists: live.has(key), resolvedVia: 'git' }];
}
