...
// T1: THEME is a named export and is an object with exactly 16 keys
test('THEME is exported as an object with exactly 16 keys', () => {
  assert.equal(typeof THEME, 'object');
  assert.notEqual(THEME, null);
  assert.equal(Object.keys(THEME).length, 16);
});

// T2: FIGLET_FONT is a named export with value 'ANSI Shadow'
test("FIGLET_FONT is 'ANSI Shadow'", () => {
  assert.equal(FIGLET_FONT, 'ANSI Shadow');
});

// T3: Each of the 15 function tokens is callable
const functionKeys = [
  'banner', 'bannerBorder', 'tagline', 'heading', 'rule',
  'label', 'body', 'secondary', 'success', 'warning',
...
// T4: THEME.spinner is the string 'cyan'
test("THEME.spinner === 'cyan'", () => {
  assert.equal(THEME.spinner, 'cyan');
});
...
// T8: sectionHeader output contains emoji and title text
test("sectionHeader('🚀', 'Getting Started') output contains emoji and title", () => {
  const logs = [];
  ...
  assert.ok(output.includes('🚀'), 'output should contain the emoji');
