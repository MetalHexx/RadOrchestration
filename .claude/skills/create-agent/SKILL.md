...
1. **Determine the filename and name**: Lowercase, hyphenated: `{agent-name}.md` (placed in `.claude/agents/`). The `name:` field must match the filename without extension (e.g., filename `tactical-planner.md` → `name: tactical-planner`) — Claude Code spawns agents by this name.
