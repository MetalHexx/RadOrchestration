...
test('draftChangelog produces a section with the new version heading', async () => {
...
test('commitRelease stages every carrier and lands one commit with the release subject', async () => {
  const log = [];
  await commitRelease({
    repoRoot: '/repo',
  ...
  // Exactly one git commit invocation (atomicity)
