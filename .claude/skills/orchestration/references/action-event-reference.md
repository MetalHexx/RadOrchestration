...
| 9 | `spawn_code_reviewer` | Agent spawn | **Two-step protocol:** (1) Signal `code_review_started` with `{}` context → pipeline returns `spawn_code_reviewer` again; (2) Spawn **reviewer** agent for task-level code review. Output: reports/{NAME}-CODE-REVIEW-P{NN}-T{NN}-{TITLE}.md | `code_review_completed --doc-path <output-path>` |
