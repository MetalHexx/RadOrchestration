...
describe('context-enrichment manifest contract', () => {
  it('spawn-suffix rendering, agent docs, action-event reference, context-enrichment.ts', () => {
    ...
    // under `## Repository Skills Available` followed by the orientation sentence,
    // OR omits the heading entirely when the array is empty.
    ...
    assert.equal(renderSpawnSuffix(raw), '', 'empty manifest → no heading appears in spawn prompt');
    ...
    assert.match(suffix, /^## Repository Skills Available$/m, 'heading must be the exact literal');
    assert.match(suffix, /Entries above are a catalog\. Read a listed path \*\*only when\*\*/, 'orientation sentence must match the read-on-match contract verbatim');
    assert.match(suffix, /outside this catalog .* was filtered on purpose; do not Read it/, 'orientation sentence must reinforce manifest authority');
