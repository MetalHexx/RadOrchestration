...
**(a)** Remove any prior `dogfood-marketplace/plugins/rad-orchestration-copilot-cli/` (so a stale copy never lingers across rebuilds), then copy the current `output/` tree into place. PowerShell:

```powershell
$mkt = "{repoRoot}\greenfield\harness-installers\copilot-cli-plugin\dogfood-marketplace"
$pluginDest = "$mkt\plugins\rad-orchestration-copilot-cli"
...
```json
{
  "name": "rad-orchestration-copilot-cli-dogfood",
  ...
  "plugins": [
    {
      ...
      "source": "./plugins/rad-orchestration-copilot-cli",
...
> copilot plugin install rad-orchestration-copilot-cli
> ```
>
...
- **Options:**
  - **Yes, clean up** — "Remove `{repoRoot}\greenfield\harness-installers\copilot-cli-plugin\dogfood-marketplace` (deletes both the marketplace.json and the plugin copy at `plugins/rad-orchestration-copilot-cli/`). The canonical build output at `output/` is left untouched. Next run of this skill regenerates the marketplace and the plugin copy from `output/`."
...
Run it. Confirm the directory is gone. Print "Cleaned up the dogfood marketplace (marketplace.json + the plugin copy at `plugins/rad-orchestration-copilot-cli/`). The greenfield plugin output at `greenfield/harness-installers/copilot-cli-plugin/output/` is untouched (rebuild via `node greenfield/harness-installers/copilot-cli-plugin/build-scripts/build.js` if needed). Re-run this skill to regenerate the dogfood marketplace."

If the user picks **Leave it for another round**:

Print the cleanup command above so they can run it later. Also remind them to run `copilot plugin uninstall rad-orchestration-copilot-cli` (and optionally `copilot plugin marketplace remove rad-orchestration-copilot-cli-dogfood`) inside the test Copilot CLI session before switching channels.
