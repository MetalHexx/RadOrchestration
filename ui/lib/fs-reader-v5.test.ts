...
function makeV5State(overrides: Record<string, unknown> = {}) {
  return JSON.stringify({
    ...
    graph: {
      ...
      nodes: {
        research: { status: 'completed', label: 'Research' },
        prd: { status: 'completed', label: 'PRD' },
        design: { status: 'completed', label: 'Design' },
        architecture: { status: 'completed', label: 'Architecture' },
        master_plan: { status: 'completed', label: 'Master Plan' },
        phase_loop: { status: 'in_progress', label: 'Phase Loop' },
        final_review: { status: 'not_started', label: 'Final Review' },
      },
    },
    ...overrides,
  });
    ...
    makeV5State({
      ...
      graph: {
        ...
        nodes: {
          ...
          architecture: { status: 'completed', label: 'Architecture' },
          master_plan: { status: 'completed', label: 'Master Plan' },
          phase_loop: { status: 'completed', label: 'Phase Loop' },
          final_review: { status: 'completed', label: 'Final Review' },
        },
      },
    })
  );
