...
import { PendingChangesBadge } from "@/components/badges";
...
export function EditableSlotCard(props: Props) {
  const { content, setContent, dirty, save, discard } = useSlotEditor(
    props.slotKey, props.onDirtyChange, props.onPersistComplete,
  );
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between" data-testid="slot-card-header">
        <CardTitle className="text-sm">{props.slotLabel}</CardTitle>
        {dirty && <PendingChangesBadge />}
      </CardHeader>
      <CardContent>
        <Textarea
          ...
          onChange={(e) => {
            ...
            props.onContentChange?.(
              `${props.slotKey.kind}.${props.slotKey.name}.${props.slotKey.slot}`,
              e.target.value,
            );
          }}
        />
