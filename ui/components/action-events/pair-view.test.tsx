...
test('applicableSlotsFor returns 5 entries for action with completion_event (FR-8)', () => {
  const slots = applicableSlotsFor({ kind: 'action', name: 'exec', completion_event: 'done' } as any);
...
test('applicableSlotsFor returns 2 entries for terminal action (FR-9)', () => {
  const slots = applicableSlotsFor({ kind: 'action', name: 'halt', completion_event: null } as any);
  assert.strictEqual(slots.length, 2);
  assert.deepStrictEqual(slots.map((s) => s.role), ['custom-action-pre', 'shipped-action']);
});

test('applicableSlotsFor returns 3 entries for orphan event (FR-9)', () => {
  const slots = applicableSlotsFor({ kind: 'event', name: 'lonely' } as any);
