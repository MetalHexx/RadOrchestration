...
test('unbound shows a placeholder watermark, no grey helper, no warning (FR-7, DD-2)', () => {
  const c = pathCardCopy('unbound');
  assert.ok(c.placeholder);
  assert.strictEqual(c.warning, undefined);
});

test('missing shows the re-point warning + red treatment (FR-7, DD-2, DD-7)', () => {
  const c = pathCardCopy('missing');
  assert.ok(c.warning && /no longer exists/i.test(c.warning));
  assert.strictEqual(c.isInvalid, true);
});

test('bound shows neither placeholder nor warning (FR-6)', () => {
  const c = pathCardCopy('bound');
  assert.strictEqual(c.placeholder, undefined);
  assert.strictEqual(c.warning, undefined);
  assert.strictEqual(c.isInvalid, false);
});

test('the grey helper subtitle is removed from the card (watermark + red error/warning carry it)', () => {
  // No grey muted helper paragraph; only the red warning + FieldError remain.
  assert.doesNotMatch(src, /text-muted-foreground/);
  assert.doesNotMatch(src, /Point this repo at your local clone/);
});
