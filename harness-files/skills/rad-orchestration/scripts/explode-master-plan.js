...
// harness-files/skills/rad-orchestration/scripts/lib/explode-master-plan.ts
...
function parseMasterPlan(masterPlanPath) {
  let raw;
  try {
    raw = fs2.readFileSync(masterPlanPath, "utf-8");
  } catch (err) {
    const code = err?.code;
    if (code === "ENOENT") {
  ...
  const doc = readDocument2(masterPlanPath);
  if (doc === null) {
    throw new ParseError({
      line: 1,
      expected: "Master Plan file at " + masterPlanPath,
...
function renderDoc(frontmatter, body) {
  const frontmatterYaml = jsYaml.dump(frontmatter, { lineWidth: 120, noRefs: true }).trimEnd();
  return `---
${frontmatterYaml}
---
...
function moveContentsTo(srcDir, destDir) {
  ...
  for (const entry of fs2.readdirSync(srcDir)) {
    const srcPath = path2.join(srcDir, entry);
    const destPath = path2.join(destDir, entry);
    try {
      fs2.renameSync(srcPath, destPath);
    } catch (err) {
      const code = err?.code;
      if (code === "EXDEV" || code === "EPERM" || code === "ENOTEMPTY") {
        fs2.cpSync(srcPath, destPath, { recursive: true });
        fs2.rmSync(srcPath, { recursive: true, force: true });
      } else {
        throw err;
      }
    }
...
function makeBackupDir(projectDir, nowIso) {
  ...
  return path2.join(projectDir, "backups", stamp);
}
function explodeMasterPlan(opts) {
  ...
  const phasesDir = path2.join(projectDir, "phases");
  const tasksDir = path2.join(projectDir, "tasks");
  let backupDir = null;
  ...
  if (phasesHas || tasksHas) {
    ...
    if (phasesHas) moveContentsTo(phasesDir, path2.join(backupDir, "phases"));
    if (tasksHas) moveContentsTo(tasksDir, path2.join(backupDir, "tasks"));
  }
  fs2.mkdirSync(phasesDir, { recursive: true });
  ...
  for (const phase of parsed.phases) {
    ...
    const fpath = path2.join(phasesDir, fname);
    ...
    fs2.writeFileSync(fpath, renderDoc(frontmatter, renderPhaseBody(phase)), "utf-8");
    emittedPhaseFiles.push(fpath);
    for (const task of phase.tasks) {
      const tname = taskFilename(projectName, task);
      const tpath = path2.join(tasksDir, tname);
      const tfront = buildTaskFrontmatter({ projectName, task, createdIso: nowIso });
      fs2.writeFileSync(tpath, renderDoc(tfront, renderTaskBody(task)), "utf-8");
...
// harness-files/skills/rad-orchestration/scripts/explode-master-plan.ts
