...
2. The very first call's `{orchRoot}` is supplied by `radorch project context` output, read from `data.orchRoot` in the envelope (the subcommand that replaces the legacy `gather-context.js`).
