...
- **Side-projects live outside the registry.** Personal, local-only projects (`side_project: true`, auto-provisioned at `~/.radorc/side-projects/<name>/`, never pushed) are intentionally **not** registered — the registry is the team's shared map of durable, remote-backed repos. Don't register them; they won't appear in `repo list`.
