...
- **Side-projects (no registered repo).** Personal or throwaway work that touches no registered team repo is a **side-project**: record `side_project: true` instead of a repo set. The system auto-provisions a local-only repo (never pushed) — don't invent a registry name for it. 
