...
- **Path (exact filename)**: `~/.radorc/projects/{PROJECT-NAME}/{PROJECT-NAME}-BRAINSTORM.html` — the visual companion to `BRAINSTORMING.md` (one per project; regenerating overwrites it).  Use this **exact** name — not `-BRAINSTORM-VISUAL.html` or any other suffix.  The dashboard fills its "Brainstorm Visual" slot only from this name; a deviation still shows as a generic visual but won't fill that slot.
