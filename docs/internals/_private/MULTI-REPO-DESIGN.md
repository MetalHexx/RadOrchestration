...
The registry's logical map is the first layer of a larger structure: as the design matures it extends into a **work-graph** — a connective **awareness layer** that links repos, projects, and worktrees into a **mind map of the developer's (and the team's) work**, deliberately open to external links (Jira, GitHub). The registry tells an agent which repos exist; the work-graph tells it what work is happening across them — and the later worktree and dashboard projects *consume* it.
...
| Command | Purpose |
|---|---|
| `radorch worktree create --project <name> [--worktree-name <name>] [--repo <name>]` | Provision worktrees for the project — one per repo in its `repos:` set under `~/.radorc/worktrees/<worktree_name>/<repo>/` (or one with `--repo`). Convention-bound (no `--worktree-path`), multi-worktree, idempotent. |
...
The `worktree` noun owns the **physical lifecycle** — `create` / `remove` / `list` / `launch` — symmetric and convention-bound. (Earlier drafts folded creation into `source-control init` and dropped the `create` verb; MULTI-REPO-4 reversed that for CLI consistency, replacing the old *ad-hoc-path* `create` interface with a convention-bound one rather than removing the verb.)

**Source-control flow** (covered in detail in the sections noted; listed here for surface completeness):

| Command | Purpose | Section |
|---|---|---|
| `radorch source-control init --project <name>` | **Check** worktrees are present + **record** `pipeline.source_control` state per repo (does **not** create — provisioning is `worktree create`) | Worktree Lifecycle + Source-Control Init |
...
**Behavior** (revised 2026-06-01 — supersedes the original "infer-hard" sketch; see the Amendment in the MULTI-REPO-1 Requirements/Brainstorming):
...
This subsection supersedes the earlier "infer-hard, last-path-segment name, one-confirmation" sketch and discharges the **Future Work** entry "`/rad-repo` skill scope."
...
| Trigger | `/rad-execute` routes into `rad-source-control`'s worktree reference (main session, no subagent spawn), which runs a **`locate`-driven decision flow** to resolve `worktree_name` / in-place / pull-missing-repo (see **Worktree reuse** below), then makes **two mechanical CLI calls**: `radorch worktree create` (provision) → `radorch source-control init` (check + record). Both are pure mechanics — no LLM work. |
| What the two CLI calls do | **`worktree create --project X [--worktree-name <name>] [--repo <name>]`** reads the project's `repos:` set, looks up each source clone from `registry.local.yml`, and runs `git worktree add ~/.radorc/worktrees/<worktree_name>/<REPO>/ <branch>` for each repo that doesn't already exist (idempotent; per-repo result array). **`source-control init --project X`** then verifies the worktrees are present, derives branch / base_branch / remote_url per repo, and writes `pipeline.source_control` directly — it does **not** create. For a side-project, provisioning is `side-project init` instead of `worktree create`; for `--in-place`, no worktree is created and init records the main-clone binding. |
...
| Worktree reuse | **Location-driven, not scan-and-prompt** (refined in the MULTI-REPO-4 brainstorm). The launch flow calls a **`locate`** classifier (cwd → `worktree` \| `main-clone` \| `side-project` \| `none`; see the new primitive in **Pipeline Data Flow → Path resolution**) and branches on a five-case flow: fresh init (`none`/main-clone); side-project (delegate to `side-project init`); standing in the **target's** worktree → reuse silently; standing in **another** project's worktree → **confirm**, then reuse its `worktree_name` on the **same branch** (the follow-up-correction case); standing in a main clone and declining a worktree → **in-place** (work the main clone on its current branch); reused set missing a needed repo → **ask**, then pull it in under the same `worktree_name`. Reuse never auto-fires — it is confirmed. Still no formal parent-child tracking in state, and `radorch worktree remove` does not know about aliased dependents. |
| In-place mode | A project may run **in the main clone with no worktree** (user declines one while standing in it). State stores the repo name + an `in_place` marker; the path is derived from `repo-registry.local.yml` at runtime, so `state.json` stays path-free. In-place is inherently this-machine and non-portable; scoped to single-repo / the standing repo (multi-repo in-place is ambiguous, deferred). |
...
> **⚠️ Known gap — registry-vs-worktree path resolution (hit live during MULTI-REPO-3; fix in a later project).**
>
> This section describes the *target* state: the engine hands each spawn a per-action `repos[]` array carrying convention-derived **worktree** `path` values (see `execute_task` shape below), and agents join relative paths against those. That enrichment lands in **MULTI-REPO-4/5** (managed worktree lifecycle + `context-enrichment.ts` per-action `repos` arrays). It does **not** exist yet in MULTI-REPO-3.
>
...
>
> **Real fix:** exactly what this section already specifies — enrichment carries `repos[].path` (worktrees) and the orchestrator inlines them into every spawn prompt (MULTI-REPO-4/5). **Interim workaround (in force now):** the orchestrator hard-codes the worktree path into every spawn prompt (coder, source-control, reviewer) and explicitly forbids touching the main clone. **The next project should also consider a guardrail** beyond prose — e.g. `source_control_init` binds the repo to the worktree for the run, or the CLI fails loud when an agent writes outside the project's worktree.
>
...
> **Scope note (this is MULTI-REPO-5, existing architecture).** Everything in this section is delivered by **MULTI-REPO-5 on the existing source-control architecture** — the `rad-source-control` agent, the `commit` node, and the `commit_completed` event/mutation all stay, only made per-repo. The later **source-control commit efficiency refactor** (MULTI-REPO-7) changes *how commits are produced and recorded* — coder self-commit + `radorch git commit` writing state directly + retiring `commit_completed` + dropping the per-task agent spawn — but that is out of scope for MULTI-REPO-5. See Delivery Sequence step 9.
...
- **Failure handling (MULTI-REPO-5 lock):** on **any** failure, **halt + tell the user + offer retry**, with an "already-open PR?" guard so retry is safe (skips a sibling whose PR already exists) — no elaborate idempotent-recovery machinery. (An earlier draft specified full idempotent re-run + pass-2-only retry; MULTI-REPO-5 simplified this to halt-and-offer-retry.)
...
- Stays **thin**: routes into `rad-source-control`'s `working-with-worktrees.md` (which runs the `locate`-driven five-case flow), instead of gathering field values + signaling a state-setter event. The CLI does the work, writes state directly, returns a result envelope.
...
- **Restructured (MULTI-REPO-4) into a routed "act" skill** — a thin `SKILL.md` router over per-operation references: `creating-commits.md` + `working-with-prs.md` (commit/PR relocated **verbatim** in MULTI-REPO-4) + new `working-with-worktrees.md` (the interactive, main-session worktree workflow that drives the two-call `worktree create`/`remove` + `source-control init`). Routing is **by operation**; the invoker is a safety annotation (subagent commit/PR modes never reach the interactive worktree flow). Stays `user-invocable: false`; the "all inputs come from the spawn prompt" promise demotes to a per-mode property. Triad note flips: worktree lifecycle now lives **in** the act lane.
- Commit/PR **fan-out** (below) completes in **MULTI-REPO-5**, rewriting `creating-commits.md` / `working-with-prs.md` in place — no router change.
- Commit mode (MULTI-REPO-5): agent reads the spawn-prompt's `repos` array, crafts per-repo commit messages in one LLM pass, returns an array of `{ name, message }`. CLI iterates per repo. **This is the existing source-control agent made multi-repo-aware — the agent, the `commit` node, and the `commit_completed` event/mutation all stay.**
- PR mode (MULTI-REPO-5): agent crafts per-repo PR descriptions in one LLM pass, returns an array of `{ name, description }`. CLI iterates per repo, then runs the two-pass linked-PR creation.
- **Forward note (MULTI-REPO-7):** the **source-control commit efficiency refactor** later retires the per-task commit spawn entirely — the coder self-commits and `radorch git commit` writes the SHA directly to `state.json`, dropping `commit_completed`. That is a separate project (delicate corrective-task coupling; benefits single-repo too); MULTI-REPO-5 leaves the commit agent in place.

### `/rad-project` (NEW)

Introduced by the work-graph consumer (PROJECT-GRAPH-2). A worldview-first, **user-invocable** skill that fronts the work-graph CLI (`project` / `project-group` / `graph`) and teaches an agent to act as a **PM / tech lead** — orient across the work, organize it (groups + relationships), and resolve *where to work* (worktree **awareness**, ask-the-tool; it never creates worktrees). Armed on the `planner` + the primary session; a lean `SKILL.md` + four references (`concepts`, `cli-commands`, `interacting-with-users`, `where-to-work`). Forms the **triad** with `rad-repo` (identity) and `rad-source-control` (git action) via short cross-reference notes. Its no-arg form returns a summary of the active projects. Detail: `~/.radorc/projects/PROJECT-GRAPH-2/`.

### `/rad-execute-parallel`

- **Largely subsumed by MULTI-REPO-4:** once it calls `radorch worktree create` (which provisions N worktrees from the project's `repos:`) followed by `radorch source-control init`, multi-repo support comes for free — the prior "extension for multi-repo" deferral is closed. Per-project worktree isolation is now **inherent** in the canonical `~/.radorc/worktrees/<PROJECT>/` convention (every project already gets its own set), so the skill's remaining distinct value is the **launch-in-a-window UX** (`worktree launch` + `post_action`), not worktree creation. Like `/rad-execute`, it thins to routing into `working-with-worktrees.md`.
...
> **Delivery order ≠ project number.** Steps below are in dependency order; each names its project. The work-graph projects PROJECT-GRAPH-1/2 sit between MULTI-REPO-3 and MULTI-REPO-4.

This design is too large to ship as one project. Practical delivery order, super high level — each step is a discrete future brainstorming + planning project. Each builds on the prior; ordering is forced by dependency.

1. **MULTI-REPO-1** — registry foundation: `repo-registry.yml` + `.local.yml` files, `cli/src/lib/repo-registry/` lib module, `radorch repo` / `radorch repo-group` CLI subcommands (including `bind`), minimal `/rad-repo` skill, session-start hook for ambient awareness. Ships standalone — no project integration yet. Unlocks everything else.

   **Bridge — UI registry management.** MULTI-REPO-1 built `lib/repo-registry` as UI-*ready* source but explicitly deferred the build/packaging wiring and any UI consumer ("the library is designed to be UI-consumable; wiring the UI onto it — and any workspace/Next.js build setup — is future work"). Two small, focused follow-on projects discharge that deferred work, in order. They branch off MULTI-REPO-1 and are **independent of the planning/execution backbone (MULTI-REPO-2/3 and MULTI-REPO-4 through MULTI-REPO-6)**, which continues unchanged — though the later work-graph projects (PROJECT-GRAPH-1/2) *depend on* MULTI-REPO-1-LIB, since `@rad-orchestration/work-graph` is itself a workspace package built on `@rad-orchestration/repo-registry`:
...
2. **MULTI-REPO-2** — multi-repo planning artifacts, authoring only. `/rad-brainstorm` gains a mandatory `## Repo Targets` section. `/rad-create-plans` gains **planner-authored** `repos:` frontmatter on Requirements (non-authoritative restate) + Master Plan (sealed) plus optional `repo-group:` provenance, per-task `**Target repos:**` + per-repo `**Files for <repo>:**` (replacing flat `**Files:**`, uniform even for single-repo), and a per-phase `**Target repos:**` union line mirroring the existing `**Requirements:**` convention. `/rad-plan-audit` gets a **minimal** update only — flip its `**Files:**`-mandatory rule to recognize the new shape so it stops mis-flagging multi-repo plans; **no** subset/union/registry validation here (that hard-gates in MULTI-REPO-3). Scope is bounded by the **author boundary**: everything stamped `author: "planner-agent"` is in; nothing the explosion script stamps is touched. Users can *author* fully multi-repo-shaped plans; the new fields are **inert** (no runtime consumes them yet), so single-repo execution is unaffected. **No `explode-master-plan.ts` change, no state-shape change, no v6 bump.**

3. **MULTI-REPO-3** — explosion + state v6. `cli/src/lib/explode-master-plan.ts` gains the **strict** `extractTargetRepos` parse (throws `ParseError` on a missing `**Target repos:**`), derives the exploded phase/task frontmatter `repos:` (phase = union of its tasks), and seeds `TaskIterationEntry.repos`; the state schema bumps `v5 → v6`. This is where the authored multi-repo shape first becomes **machine-enforced** and runtime-visible, and the home for the real subset/union/registry validation `/rad-plan-audit` deferred in MULTI-REPO-2. After this, an authored multi-repo plan explodes into per-repo handoffs + per-repo state. Because the v6 shape lands here, MULTI-REPO-3 also carries a **minimal engine shim** — the commit-write mutation and review-SHA enrichment move onto the `repos[]` array but handle only the single-repo case (`repos[0]`), and corrective-task entries collapse alongside task entries; rich multi-repo enrichment (and removal of this shim) is MULTI-REPO-5.

   **Follow-up — `MULTI-REPO-3-SIDE-PROJECTS` (typed `project-type` + local-only side-projects).** Surfaced during MULTI-REPO-3: a self-contained project that touches *no registered repo* (a trial `MULTI-REPO-3-TEST` run) had no graceful way to declare itself, so the planner fabricated the project name as a repo and `/rad-plan-audit` flagged it (§2.5/§2.6). Decision: introduce a typed **`project-type`** frontmatter field — an enum applied to *every* project, with `standard` the default kind (absent ⇒ `standard`, so existing projects need no migration) and `side-project` the first specialized value (extensible to future kinds, e.g. `follow-up`). A **side-project** is an **auto-provisioned, local-only git repo** at `~/.radorc/side-projects/<project>/` — a **third convention-resolved location-kind** beside registered repos (`.local.yml` lookup) and worktrees, **unregistered by design** (the registry is the team's shared, versioned map; side-projects are personal). `repos:` carries the project's own name — a *real* local repo, so the original "fabrication" becomes correct — with `repo-group: null`, mutually exclusive with registered repos; the project's planning docs + state still live under `~/.radorc/projects/<project>/` like any project (only the code repo lives under `side-projects/`, so the dashboard discovers it for free). Execution rides the **ordinary single-repo path** with kind-gated overrides: a new **`radorch side-project init`** command (mirroring `worktree create`) runs `git init` + a seed commit at the convention path and feeds `worktree_path` into `source-control init`; `auto_commit: always`, `auto_pr: never`; and `radorch git commit` **auto-skips the push when there is no `origin` remote** (a clean `pushed:false`, distinct from `push_failed`). Touch-points: the launch skills (`rad-plan` Step 6, `rad-approve-plan`, `rad-execute` Step 3, `rad-execute-parallel`) gate their git-strategy prompts on `project-type`, read via `radorch project context` (extended to surface the field — never the master-plan body); the planner (`rad-create-plans`) stamps the kind on every plan and seals a side-project's `repos: [<name>]` / `repo-group: null` without a registry lookup; the auditor skips registry-membership for a side-project's own repo; `project_type` is an **additive optional** field on `state.project` in the v6 schema (non-breaking, no migration), seeded during explosion; `side-projects/` is **sacred + gitignored** (nested repos); `/rad-brainstorm` and `/rad-repo` gain side-project awareness and `rad-source-control` notes the clean local-only commit; the dashboard adds a "Local · side-project" badge; and the new command ships with `--help` at all depths plus a help-shape regression-test block. Additive overall — normal multi-repo plans are untouched. Branches off MULTI-REPO-3.

4. **PROJECT-GRAPH-1** — work-graph backend, the spine. A new `@rad-orchestration/work-graph` library **and its full CLI surface** — three `radorch` nouns, `project` / `project-group` / `graph` (depends on `@rad-orchestration/repo-registry`) — that turns the flat list of ~180 projects into **one queryable graph** — **nodes + typed edges**: **Groups** (stored) and **Projects** (derived), with their **worktrees**; phases and tasks are future node kinds. It is an **overlay** — it persists only Groups and Edges in a **central, versioned store** (`work-graph.yml`, YAML, mirroring `repo-registry.yml`) and derives everything else live: projects by scanning disk, worktree locations from `git worktree list` + convention, status from each `state.json` — never duplicating `state.json`. Object-oriented core (a `Node` interface, `Edge` value objects, a `WorkGraph` aggregate, a `GraphIndex` repository that solely owns the store, and a `WorkGraphService` facade); it returns a friendly **versioned JSON projection** and **validates every write** so the CLI/UI cannot corrupt the graph. The edge vocabulary is a known-but-extensible set, seeded with `contains` (structure) plus `spawned-from` and `follows` (relationships). Its `project` verbs reach **parity** with the legacy `radorch project` so it can be retired in PROJECT-GRAPH-2. **PROJECT-GRAPH-1 owns building the entire CLI surface** — the commands, their lean agent-facing output, the standard envelope, and tiered `--help`; PROJECT-GRAPH-2 only *consumes* it. Full detail lives in the project docs at `~/.radorc/projects/PROJECT-GRAPH-1/` (`PROJECT-GRAPH-1-BRAINSTORMING.md` + `PROJECT-GRAPH-1-TECHNICAL-DESIGN.md`).

5. **PROJECT-GRAPH-2** — work-graph consumer; **consumes the PROJECT-GRAPH-1 CLI surface** (it adds no new library or command capability). Builds **ambient awareness** (priming agents with live project + worktree state — today's session hook carries only the repo map, and only to the primary agent) and the **`rad-project` skill** — a worldview-first, agent-as-PM / tech-lead skill driving the `project` / `project-group` / `graph` commands, armed on the `planner` + the primary session — together delivering the "the agent knows everything that's going on" experience. `rad-project` joins `rad-repo` (identity) and `rad-source-control` (git action) as a cross-referenced **triad** (orient → situate → act).

   **Migration is partial by design — the system stays fully working at every step.** PROJECT-GRAPH-2 **retires `project find`** (callers move to `project list` / `show`; note the *tier→status* shift, resolved with a client-side `tier` filter, and the `masterPlanPath` → `docs.masterPlan` + `dir` composition), **removes the vestigial `radorch where`** (no runtime consumer), **adds `radorch config`** (owns the `orchestration.yml` config values; feeds the ambient Config row; callers call it on demand), and migrates the *simple* `project context` callers (the single-field `projectType` reads in `rad-plan` / `rad-approve-plan` / `rad-execute`) onto `project show`. **`project context` is deliberately NOT fully removed here** — it stays the home for `rad-execute`'s `source-control-init` flow (a three-way fan-out of project + config + git/workspace facts), whose migration and `project context`'s final retirement complete in **MULTI-REPO-4**, where that flow is rebuilt anyway. **Source-control proper is out of scope here:** the `rad-source-control` rewrite into the triad's "act" lane defers to MULTI-REPO-4 and MULTI-REPO-5 (worktree lifecycle, commit/PR fan-out). Cross-cutting: the triad cross-reference mesh (`rad-project`'s note + a reciprocal in `rad-repo`) and the **harness-installer manifest** updates for the new skill + `planner` change + the new/removed commands. Full detail lives in the project docs at `~/.radorc/projects/PROJECT-GRAPH-2/` (`PROJECT-GRAPH-2-BRAINSTORMING.md` + `PROJECT-GRAPH-2-TECHNICAL-DESIGN.md`).

6. **MULTI-REPO-4** — worktree + source-control init, split into **two clean CLI calls** by noun. The **`worktree` noun** gains a symmetric physical lifecycle — `create` + `remove` (plus existing `launch`), all **convention-bound** (`--project` / `--worktree-name` / `--repo`; no ad-hoc `--worktree-path`), **multi-worktree** (one call provisions/removes every repo in the set), and **idempotent**; canonical `~/.radorc/worktrees/<PROJECT>/<repo>/` wired into `userDataPaths`; side-projects route to `side-project init`. **`radorch source-control init`** is reframed from a state-setter *event* into a real CLI command that **checks the worktrees are present and records `pipeline.source_control` state directly** (the v6 `repos[]` + `worktree_name` shape) — it does **not** create. The launch flow makes two calls: `worktree create` → `source-control init`. After this, multi-repo projects can be initialized end-to-end on disk. **Also new:** a **`locate`** reverse-lookup primitive (cwd → project/worktree/clone classifier, in the work-graph lib, generalizing PG-2's `resolveYouAreIn`) driving a **location-aware five-case init flow** (fresh / side-project / reuse-silent / reuse-confirm-same-branch / **in-place** / pull-missing-repo — where pull-missing-repo is simply `worktree create --repo <missing>`). The old **ad-hoc-path `worktree create` interface is reworked** to the convention-bound one above (its `worktreeCreate()` function is reused) — the verb stays, the footgun (arbitrary `--worktree-path`) goes. This is also where the **`rad-source-control`** skill is **restructured into a routed "act" skill** — `SKILL.md` router + `creating-commits.md` + `working-with-prs.md` (commit/PR relocated **verbatim**) + new `working-with-worktrees.md` (drives `worktree create`/`remove` + `source-control init`); commit/PR fan-out rewrite completes in MULTI-REPO-5. Because MULTI-REPO-4 rebuilds `rad-execute`'s `source-control-init` flow, it **completes the `project context` retirement deferred from PROJECT-GRAPH-2** — moving that flow's project + config + git/workspace reads onto `project show` / `radorch config` / git-derived sources — after which `project context` is removed and the legacy `project` verbs are fully retired. Full detail: `~/.radorc/projects/MULTI-REPO-4/` (`MULTI-REPO-4-BRAINSTORMING.md` + `MULTI-REPO-4-TECHNICAL-DESIGN.md`).

7. **MULTI-REPO-5** — full multi-repo execution close: write → review → commit → PR, all per-repo, **on the EXISTING source-control architecture** (the source-control agent, the `commit` node, and the `commit_completed` event/mutation all stay — only made per-repo). `context-enrichment.ts` produces per-action `repos` arrays, catalog `.md` bodies updated, coder consumes per-repo `**Files for <repo>:**` + `repos[N].path`, reviewer iterates per-repo SHAs per scope (task / phase / final) and adds a boundary sanity-check, **commit fan-out** uses the existing `rad-source-control` agent (commit mode, made multi-repo-aware) — ONE spawn per task crafting per-repo conventional-commit messages in a single LLM pass, with `radorch git commit` iterating per repo and per-repo SHAs recorded via the existing `commit_completed` event/mutation (made per-repo) — and **PR fan-out** uses the same agent (PR mode) for per-repo PR descriptions with `radorch git pr` running two-pass linked-PR creation (halt + tell the user + offer retry on any failure, with an "already-open PR?" guard). This step **removes the ENTIRE interim shim** — the MULTI-REPO-3 single-repo `repos[0]` engine handling *and* MULTI-REPO-4's top-level `worktree_path` / `branch` / `base_branch` compat fields — moving all three enrichment readers (planner, commit, PR) onto per-repo `repos[]` at once. Closes the **execution** loop to full multi-repo end-to-end. **The source-control commit efficiency refactor (coder self-commit, `radorch git commit` writing state directly, retiring `commit_completed`, killing the per-task source-control spawn) is explicitly OUT of scope here — it lands in MULTI-REPO-7.** *(This entry absorbs the former standalone MULTI-REPO-6 source-control fan-out, folded in during a 2026-06-09 pass — a multi-repo run isn't usable until commits land per-repo, and the PR work is the same shape. MULTI-REPO-4's sealed docs predate this renumber and still forward-ref the old numbers — "fan-out → MULTI-REPO-6", "dashboard → MULTI-REPO-7"; this design doc is the source of truth, so read those as MULTI-REPO-5 and MULTI-REPO-6 respectively.)*

8. **MULTI-REPO-6** — multi-repo dashboard: the radorch dashboard renders per-repo state across the projects timeline: per-repo commit hashes on task cards, the per-repo source-control section (branch / compare / PR URL per repo), and the combined multi-repo review + linked-PR surfaces. Lands **immediately after MULTI-REPO-5**, once all the per-repo data exists to display (task commits from MULTI-REPO-3's v6 state, source-control from MULTI-REPO-4, fan-out commits + linked PRs from MULTI-REPO-5). MULTI-REPO-6 also **removes the temporary minimal-rendering shim** introduced in MULTI-REPO-3 — the lossy multi-repo task-commit display added there solely to keep the dashboard from breaking on v6 state (single-repo rendered correctly; multi-repo rendered minimally). Until MULTI-REPO-6 lands, the dashboard is *honest but not rich* for multi-repo projects.

9. **MULTI-REPO-7** — source-control commit efficiency refactor (an architecture change + token optimization, **not** a multi-repo feature). Eliminate the per-task source-control agent spawn (~8–15k tokens per commit) by having the **coder self-commit** at the end of its task (gated on `auto_commit`, which must be **surfaced into the `execute_task` enrichment** — it is not carried there today, only `phase_id` / `task_id` / `handoff_doc` / `repos[]` are) and having **`radorch git commit` write the commit SHA directly into `state.json`** (the same direct-state-mutation pattern MULTI-REPO-4 used for `source-control init` and `explode` — no `commit_completed` event-payload round-trip). The CLI does commit + state-write **atomically: commit first, then write state**, so a failed commit can't record a phantom hash; the caller passes `--task-id` so the CLI writes to that exact iteration (simpler than the mutation's active-entry scanning). The `commit` / `commit_gate` node is removed from the **4 pipeline templates** (`runtime-config/templates/*.yml`) **reversibly**, and the `commit_completed` event + mutation are **retired (or made dormant)** — recording moves into the CLI. **Delicate (the main reason it's its own project):** `commit_completed` also fires on the **corrective-task commit path** (phase-corrective + task-corrective), with "Verify Before Echo" guarding in `harness-files/skills/rad-orchestration/references/corrective-playbook.md`; this coupling must be untangled carefully. The coder is **not** armed with the whole `rad-source-control` skill — a minimal inline commit instruction, or message-building moved into the CLI (`--task-id` / `--type` / `--title` alongside `--message`). **Applies to single AND multi-repo** — it's an optimization, not multi-repo-specific — and is sequenced at the **end** of the group because MULTI-REPO-5 must first land multi-repo execution on the existing architecture. *(Split out of MULTI-REPO-5 on 2026-06-10.)* Detail: `~/.radorc/projects/MULTI-REPO-7/MULTI-REPO-7-BRAINSTORMING.md` (scaffold — not yet brainstormed).

Topics in **Future Work** below are not assigned to specific projects — they're open questions that fall *outside* this design entirely, to be picked up when needed.

---

## Other Considerations or Future Work

The following topics were intentionally not revisited in this brainstorming pass and should be considered for each project as appropriate or saved for a future follow-up project. Some are multi-repo specific, some are general improvements that apply regardless of repo count.
...
| Project-repos discovery script | Parallel to skill-discovery: orchestrator surfaces a `## Project Repos Available` block to the planner. **Related to the work-graph projects (PROJECT-GRAPH-1/2)** — the work-graph already surfaces a project's repos + worktrees as derived nodes, so this block can be built on its read API rather than a standalone script. |
...
| Worktree-removal CLI command | ✅ **Pulled into MULTI-REPO-4** — `radorch worktree remove <project> [--repo <name>]` + the cleanup half of `working-with-worktrees.md`. Pipeline never auto-destroys; shared-`worktree_name` dependents surfaced as a risk, not formally tracked. |
...
| **Registry-vs-worktree path resolution (hit live in MULTI-REPO-3)** | Until enrichment carries `repos[].path` (MULTI-REPO-4/5), an agent that resolves a target repo *by name* via the registry lands in the **canonical main clone**, not the project worktree — wrong branch, silent divergence. See the callout in **Pipeline Data Flow → Path resolution** and `MULTI-REPO-3-ERROR-LOG.md` Error 1. Ensure MULTI-REPO-4/5 closes this with `repos[].path` in every spawn prompt, and add a guardrail beyond prose (bind repo→worktree for the run, or fail-loud on writes outside the worktree). **Related to the work-graph projects (PROJECT-GRAPH-1/2)** — the work-graph resolves a project's worktree locations from `git worktree list` + convention (not the registry's main-clone paths), so it is a natural, convention-agnostic source of the worktree-correct paths this fix needs. **MULTI-REPO-4 lays the half:** the canonical convention + the new **`locate`** classifier give a convention-derived, path-free source of truth; MULTI-REPO-5 then wires `repos[].path` into spawns from it (and removes MULTI-REPO-4's interim derived top-level `worktree_path` compat field). |

---

## Status

This direction is captured for the moment multi-repo work is picked up. Treat the locks above as direction, not finished specification — implementation will surface decisions this design intentionally leaves open. The **Future Work** table lists those open topics for the team to scope when ready.

A 2026-06-07 pass folded in the **work-graph** direction — two new projects (PROJECT-GRAPH-1, work-graph backend; PROJECT-GRAPH-2, work-graph consumer) — and renumbered the rest of the delivery sequence. All inline cross-references were updated to match.

A 2026-06-08 pass detailed **PROJECT-GRAPH-2** during the PROJECT-GRAPH-2 brainstorm — scoping the `rad-project` skill, the restyled ambient preamble (active set · config · "you're in"), and a **partial** migration: PROJECT-GRAPH-2 retires `project find` + `radorch where` and adds `radorch config`, while **`project context`'s full retirement defers to MULTI-REPO-4** (folded into the `source-control-init` reframe). PROJECT-GRAPH-2 and MULTI-REPO-4 — and the `## Agent + Skill Updates` section (new `/rad-project` entry) — were updated accordingly.

A 2026-06-09 pass detailed **MULTI-REPO-4** during its brainstorm, settling on a **two-command model** split by noun: the **`worktree` noun** owns the physical lifecycle (`create` + `remove`, convention-bound, multi-worktree, idempotent — the old ad-hoc-path `create` interface is *reworked*, not retired, restoring symmetry with `remove`), while **`radorch source-control init`** is reframed from a state-setter event into a real CLI command that **checks worktrees + records state** (does not create). The launch flow makes two calls: `worktree create` → `source-control init`. Also folded in: the canonical `~/.radorc/worktrees/<PROJECT>/<repo>/` convention (wired into `userDataPaths`); a **`locate`** reverse-lookup primitive driving a **location-aware five-case init flow** (adding **in-place** — run in the main clone, no worktree — and pull-missing-repo, which is just `worktree create --repo <missing>`, replacing the old scan-and-prompt reuse); and the `rad-source-control` **routed-skill restructure** (router + `creating-commits.md` + `working-with-prs.md` + `working-with-worktrees.md`). The **Worktree management** CLI table, the **New init flow** table, the MULTI-REPO-4/`/rad-source-control` entries, and a Future-Work row were updated to match. Forward note for MULTI-REPO-5 through MULTI-REPO-6: **in-place projects** (main-clone, no managed worktree) are a new surface for enrichment, review, and dashboard rendering.

> *Note: an interim 2026-06-09 draft had `source-control init` create worktrees and retire the `worktree create` command; this was reversed the same day for CLI noun/verb consistency — see the two-command model above.*

A 2026-06-09 pass **folded the standalone source-control fan-out project into MULTI-REPO-5** and **renumbered the dashboard from MULTI-REPO-7 to MULTI-REPO-6**. Rationale: a multi-repo run isn't usable until commits land per-repo, and the PR work is the same shape — a separate project was too much ceremony for the size. MULTI-REPO-5 is now the full execution close (coder + reviewer + per-repo commits + linked PRs) and **removes the ENTIRE interim compat shim** in one step (the MULTI-REPO-3 single-`repos[0]` engine handling *and* MULTI-REPO-4's top-level `worktree_path` / `branch` / `base_branch` fields — all three enrichment readers move to per-repo `repos[]`). The former MULTI-REPO-6 (standalone fan-out) is deleted; the former MULTI-REPO-7 (dashboard) becomes MULTI-REPO-6. The **Delivery Sequence** (steps 7–8), the `/rad-source-control` and MULTI-REPO-4 / PROJECT-GRAPH-2 entries, the MULTI-REPO-1 bridge backbone-range, the group-description delivery spine, and the work-graph store (`work-graph.yml`) were all updated to match.

A 2026-06-10 pass **split the source-control commit efficiency refactor out of MULTI-REPO-5 into a new MULTI-REPO-7** at the end of the group (chain: MR-4 → MR-5 → MR-6 → MR-7). The refactor — **coder self-commit**, **`radorch git commit` writing the SHA directly into `state.json`**, **retiring `commit_completed`** + its mutation, removing the `commit` node from the 4 pipeline templates (reversibly), and surfacing `auto_commit` into the `execute_task` enrichment — was pulled out because it is an **architecture change tangled with the corrective-task commit machinery** (`commit_completed` also fires on phase-/task-corrective paths, with "Verify Before Echo" guarding in the corrective playbook) and is a token optimization that **benefits single-repo too** — genuinely separate from "make multi-repo work." MULTI-REPO-5 stays focused on multi-repo execution end-to-end on the **existing** source-control architecture (the source-control agent + `commit` node + `commit_completed` event/mutation all stay, made per-repo). Updated to match: the **Delivery Sequence** (MR-5 entry reworded, new step 9 = MULTI-REPO-7), the **Source-Control Fan-Out** scope note + the PR failure-handling line (halt + offer retry, per the MR-5 lock), the `/rad-source-control` Agent+Skill entry (MR-7 forward note), the **group-description delivery spine** in `work-graph.yml` (with a new MULTI-REPO-7 node + `follows` edge MR-6 → MR-7), and a new scaffold project `MULTI-REPO-7/MULTI-REPO-7-BRAINSTORMING.md`. The three MULTI-REPO-5 brainstorm files (`-BRAINSTORMING.md`, `-BRAINSTORM.html`, `-LANDSCAPE.html`) were revised to frame commit/PR fan-out as the existing-agent path and to forward-point the refactor to MULTI-REPO-7.
