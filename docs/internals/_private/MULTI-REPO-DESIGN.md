...
5. **Work-graph consumer** — **consumes the iteration-4 CLI surface** (it adds no new library or command capability). Builds **ambient awareness** (priming agents with live project + worktree state — today's session hook carries only the repo map, and only to the primary agent) and a **project-management skill** that drives those commands, together delivering the "the agent knows everything that's going on" experience. Migrates the `project context` / `project find` callers and removes those legacy verbs (the `project` noun stays).

6. **Worktree + source-control init** — `radorch worktree` CLI, `worktree_name` field, managed-worktree lifecycle, `radorch source-control init` reframed as a real CLI doing the work + state mutation directly. After this, multi-repo projects can be initialized end-to-end on disk.
