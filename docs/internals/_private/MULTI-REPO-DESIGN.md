...
4. **Work-graph backend — the spine** (project `PROJECT-GRAPH-1`). A new `@rad-orchestration/work-graph` library plus a `radorch work-graph` CLI command (depends on `@rad-orchestration/repo-registry`) that turns the flat list of ~180 projects into **one queryable graph** — **nodes + typed edges**: **Groups** (stored) and **Projects** (derived), with their **worktrees**; phases and tasks are future node kinds. It is an **overlay** — it persists only Groups, Edges, and the active pointer in a **central, versioned store** (`work-graph.yml`, YAML, mirroring `repo-registry.yml`) and derives everything else live: projects by scanning disk, worktree locations from `git worktree list` + convention, status from each `state.json` — never duplicating `state.json`. Object-oriented core (a `Node` interface, `Edge` value objects, a `WorkGraph` aggregate, a `GraphIndex` repository that solely owns the store, and a `WorkGraphService` facade); it returns a friendly **versioned JSON projection** and **validates every write** so the CLI/UI cannot corrupt the graph. The edge vocabulary is a known-but-extensible set, seeded with `contains` (structure) plus `spawned-from` and `follows` (relationships). Its read API reaches **parity** with `radorch project` so that command can be retired later. Full detail lives in the project docs at `~/.radorc/projects/PROJECT-GRAPH-1/` (`PROJECT-GRAPH-1-BRAINSTORMING.md` + `PROJECT-GRAPH-1-TECHNICAL-DESIGN.md`).

5. **Work-graph consumer** — builds on the spine. The agent-facing `radorch work-graph` CLI calls, **ambient awareness** (priming agents with live project + worktree state — today's session hook carries only the repo map, and only to the primary agent), and a **project-management skill**, together delivering the "the agent knows everything that's going on" experience. Migrates the `project context` / `project find` callers and **retires `radorch project`**.
