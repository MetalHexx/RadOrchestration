...
5. **Pipeline + coder + reviewer** — `context-enrichment.ts` produces per-action `repos` arrays, catalog `.md` bodies updated, coder consumes per-repo `**Files for <repo>:**` + `repos[N].path`, reviewer iterates per-repo SHAs per scope (task / phase / final). End-to-end execution works minus commit/PR fan-out.

6. **Source-control fan-out** — per-repo commit messages (conventional commits, LLM-crafted), `radorch git commit` per-repo iteration, per-repo PR descriptions, `radorch git pr` two-pass linked-PR creation. Closes the loop to full multi-repo end-to-end.
