...
3. **Explosion + state v6** (project `MULTI-REPO-3`). `cli/src/lib/explode-master-plan.ts` gains the **strict** `extractTargetRepos` parse (throws `ParseError` on a missing `**Target repos:**`), derives the exploded phase/task frontmatter `repos:` (phase = union of its tasks), and seeds `TaskIterationEntry.repos`; the state schema bumps `v5 → v6`. This is where the authored multi-repo shape first becomes **machine-enforced** and runtime-visible, and the home for the real subset/union/registry validation `/rad-plan-audit` deferred in iteration 2. After this, an authored multi-repo plan explodes into per-repo handoffs + per-repo state. Because the v6 shape lands here, iteration 3 also carries a **minimal engine shim** — the commit-write mutation and review-SHA enrichment move onto the `repos[]` array but handle only the single-repo case (`repos[0]`), and corrective-task entries collapse alongside task entries; rich multi-repo enrichment (and removal of this shim) is iteration 5.
   ...
   **Follow-up — `MULTI-REPO-3-SIDE-PROJECTS` (side-project / local-only support).** Surfaced live during iteration 3: a self-contained project targeting *no registered repo* (a trial `MULTI-REPO-3-TEST` run) had no graceful declaration, so the planner fabricated the project name as a repo and `/rad-plan-audit` flagged it (§2.5/§2.6). Decision (reframed from an early "null-repo sentinel + captured write-location" sketch that needed pervasive special-casing): treat such work as a **side-project** — an **auto-provisioned, local-only git repo** at `~/.radorc/side-projects/<project>/`, a **third convention-resolved location-kind** beside registered-repos (`.local.yml` lookup) and worktrees, **unregistered by design** (the registry is the team's shared, versioned map; side-projects are personal). A `side_project: true` flag marks the *kind*; `repos:` carries the project's own name — now a **real** local repo, so the original "fabrication" becomes correct — with `repo-group: null`, mutually exclusive with registered repos. Execution rides the **ordinary single-repo path** with three flag-gated overrides — `git init` (not `git worktree add`), commit locally, **skip push/PR** — so most special-casing dissolves. Touch-points: the launch skills (`rad-plan` Step 6, `rad-approve-plan`, `rad-execute` Step 3, `rad-execute-parallel`) gate their git-strategy prompts on the flag (read via `radorch project context`, never the master-plan body); `/rad-brainstorm` and `/rad-repo` gain side-project awareness; the auditor skips registry-membership for the flagged repo; `side_project` is an additive optional v6 field (no migration); and `side-projects/` is **sacred + gitignored** (nested repos). Additive overall — normal multi-repo plans are untouched. Branches off iteration 3.
