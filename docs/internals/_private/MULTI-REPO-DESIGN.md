...
4. **Work-graph backend — the spine** (project `PROJECT-GRAPH-1`). A new `@rad-orchestration/work-graph` library **and its full CLI surface** — three `radorch` nouns, `project` / `project-group` / `graph` (depends on `@rad-orchestration/repo-registry`) — that turns the flat list of ~180 projects into **one queryable graph** — **nodes + typed edges**: **Groups** (stored) and **Projects** (derived), with their **worktrees**; phases and tasks are future node kinds. It is an **overlay** — it persists only Groups and Edges in a **central, versioned store** (`work-graph.yml`, YAML, mirroring `repo-registry.yml`) and derives everything else live: projects by scanning disk, worktree locations from `git worktree list` + convention, status from each `state.json` — never duplicating `state.json`. Object-oriented core (a `Node` interface, `Edge` value objects, a `WorkGraph` aggregate, a `GraphIndex` repository that solely owns the store, and a `WorkGraphService` facade); it returns a friendly **versioned JSON projection** and **validates every write** so the CLI/UI cannot corrupt the graph. The edge vocabulary is a known-but-extensible set, seeded with `contains` (structure) plus `spawned-from` and `follows` (relationships). Its `project` verbs reach **parity** with the legacy `radorch project` so it can be retired in iteration 5. **This iteration owns building the entire CLI surface** — the commands, their lean agent-facing output, the standard envelope, and tiered `--help`; iteration 5 only *consumes* it. Full detail lives in the project docs at `~/.radorc/projects/PROJECT-GRAPH-1/` (`PROJECT-GRAPH-1-BRAINSTORMING.md` + `PROJECT-GRAPH-1-TECHNICAL-DESIGN.md`).

5. **Work-graph consumer** (project `PROJECT-GRAPH-2`) — **consumes the iteration-4 CLI surface** (it adds no new library or command capability). Builds **ambient awareness** (priming agents with live project + worktree state — today's session hook carries only the repo map, and only to the primary agent) and the **`rad-project` skill** — a worldview-first, agent-as-PM / tech-lead skill driving the `project` / `project-group` / `graph` commands, armed on the `planner` + the primary session — together delivering the "the agent knows everything that's going on" experience. `rad-project` joins `rad-repo` (identity) and `rad-source-control` (git action) as a cross-referenced **triad** (orient → situate → act).

   **Migration is partial by design — the system stays fully working at every step.** Iteration 5 **retires `project find`** (callers move to `project list` / `show`; note the *tier→status* shift, resolved with a client-side `tier` filter, and the `masterPlanPath` → `docs.masterPlan` + `dir` composition), **removes the vestigial `radorch where`** (no runtime consumer), **adds `radorch config`** (owns the `orchestration.yml` config values; feeds the ambient Config row; callers call it on demand), and migrates the *simple* `project context` callers (the single-field `projectType` reads in `rad-plan` / `rad-approve-plan` / `rad-execute`) onto `project show`. **`project context` is deliberately NOT fully removed here** — it stays the home for `rad-execute`'s `source-control-init` flow (a three-way fan-out of project + config + git/workspace facts), whose migration and `project context`'s final retirement complete in **iteration 6**, where that flow is rebuilt anyway. **Source-control proper is out of scope here:** the `rad-source-control` rewrite into the triad's "act" lane defers to iterations 6 and 8 (worktree lifecycle, commit/PR fan-out). Cross-cutting: the triad cross-reference mesh (`rad-project`'s note + a reciprocal in `rad-repo`) and the **harness-installer manifest** updates for the new skill + `planner` change + the new/removed commands. Full detail lives in the project docs at `~/.radorc/projects/PROJECT-GRAPH-2/` (`PROJECT-GRAPH-2-BRAINSTORMING.md` + `PROJECT-GRAPH-2-TECHNICAL-DESIGN.md`).

6. **Worktree + source-control init** — `radorch worktree` CLI, `worktree_name` field, managed-worktree lifecycle, `radorch source-control init` reframed as a real CLI doing the work + state mutation directly. After this, multi-repo projects can be initialized end-to-end on disk. This is also where the **`rad-source-control`** skill begins its rewrite into the triad's "act" lane (worktree lifecycle here; commit/PR fan-out completes in iteration 8). Because this iteration rebuilds `rad-execute`'s `source-control-init` flow, it **completes the `project context` retirement deferred from iteration 5** — moving that flow's project + config + git/workspace reads onto `project show` / `radorch config` / git-derived sources — after which `project context` is removed and the legacy `project` verbs are fully retired.

7. **Pipeline + coder + reviewer** — `context-enrichment.ts` produces per-action `repos` arrays, catalog `.md` bodies updated, coder consumes per-repo `**Files for <repo>:**` + `repos[N].path`, reviewer iterates per-repo SHAs per scope (task / phase / final), **removing the iteration-3 engine shim** (single-repo `repos[0]` handling becomes fully per-repo). End-to-end execution works minus commit/PR fan-out.

8. **Source-control fan-out** — per-repo commit messages (conventional commits, LLM-crafted), `radorch git commit` per-repo iteration, per-repo PR descriptions, `radorch git pr` two-pass linked-PR creation. Closes the **execution** loop to full multi-repo end-to-end.

9. **Multi-repo dashboard** — the radorch dashboard renders per-repo state across the projects timeline: per-repo commit hashes on task cards, the per-repo source-control section (branch / compare / PR URL per repo), and the combined multi-repo review + linked-PR surfaces. Lands **immediately after iteration 8**, once all the per-repo data exists to display (task commits from iteration 3's v6 state, source-control from iteration 6, fan-out PRs from iteration 8). This iteration also **removes the temporary minimal-rendering shim** introduced in iteration 3 — the lossy multi-repo task-commit display added there solely to keep the dashboard from breaking on v6 state (single-repo rendered correctly; multi-repo rendered minimally). Until iteration 9 lands, the dashboard is *honest but not rich* for multi-repo projects.
...
| **Registry-vs-worktree path resolution (hit live in MULTI-REPO-3)** | Until enrichment carries `repos[].path` (iters 6–7), an agent that resolves a target repo *by name* via the registry lands in the **canonical main clone**, not the project worktree — wrong branch, silent divergence. See the callout in **Pipeline Data Flow → Path resolution** and `MULTI-REPO-3-ERROR-LOG.md` Error 1. Ensure iter 6/7 closes this with `repos[].path` in every spawn prompt, and add a guardrail beyond prose (bind repo→worktree for the run, or fail-loud on writes outside the worktree). **Related to the work-graph iterations (4–5)** — the work-graph resolves a project's worktree locations from `git worktree list` + convention (not the registry's main-clone paths), so it is a natural, convention-agnostic source of the worktree-correct paths this fix needs. |
...
A 2026-06-07 pass folded in the **work-graph** direction — two new iterations (4, work-graph backend / `PROJECT-GRAPH-1`; 5, work-graph consumer) — and renumbered the rest of the sequence (old 4–7 → 6–9). All inline iteration cross-references were updated to match.

A 2026-06-08 pass detailed **iteration 5** during the PROJECT-GRAPH-2 brainstorm — scoping the `rad-project` skill, the restyled ambient preamble (active set · config · "you're in"), and a **partial** migration: iteration 5 retires `project find` + `radorch where` and adds `radorch config`, while **`project context`'s full retirement defers to iteration 6** (folded into the `source-control-init` reframe). Iterations 5 and 6 — and the `## Agent + Skill Updates` section (new `/rad-project` entry) — were updated accordingly.
