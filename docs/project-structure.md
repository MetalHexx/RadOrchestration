...
**`BRAINSTORMING.md`** — Records validated ideas, scope boundaries, and problem statements produced during the brainstorming stage. Optional; not every project uses this stage.

**`REQUIREMENTS.md`** — A lean ledger of functional requirements (FR-), non-functional requirements (NFR-), architectural decisions (AD-), and design decisions (DD-). Every downstream step traces to IDs defined here.

**`MASTER-PLAN.md`** — High-level phase outlines, phase exit criteria, and a risk register. Input to the explosion script that generates `phases/` and `tasks/`.

**Phase plan files (`phases/`)** — One file per phase, generated by the explosion script from `MASTER-PLAN.md`. Lists the tasks in that phase and their execution order. Not hand-authored.
